...
- **Binaire statique** : pas possible car on a des services multiples (simu, EOD store, UI)
...
Flux :
```
User -> FleetSim UI -> FleetSim auth service (OIDC) -> LDAP/AD client (Kerberos/LDAPS)
                         |
                         v
