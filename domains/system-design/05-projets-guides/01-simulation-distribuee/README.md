...
FleetSim simule jusqu'a ~2000 robots autonomes (AGV, sorters, drones) en temps reel. Sur un seul serveur ca passe jusqu'a ~500 unites. Au-dela, il faut **distribuer** : partitionner l'entrepot sur N serveurs, synchroniser leurs horloges simulees, gerer les unites qui traversent les frontieres de zone. Le tout en restant **deterministe** (rejeu pour EOD Review) et **resilient** (un crash serveur ne doit pas corrompre tout le shift en cours).

Tu es l'AI Engineer sur l'equipe R&D. Le tech lead te demande une proposition d'architecture pour un prototype "FleetSim distribue" capable de faire tourner 5000 unites sur 5 serveurs en parallele.
...
| Resilience | crash d'un worker = 1 shift perdu max, pas de corruption des autres |
...
- **Pas de crash recovery** — si un worker tombe, le shift est perdu
...
- Comment un crash de la zone centrale (celle avec le plus d'activite, type docks ou tri) impacte le shift ?
...
- **OPC-UA / MQTT pub-sub** — les normes SCADA / IIoT couvrent deja une grande partie du sync inter-noeuds, pourquoi ne pas les reutiliser plutot que d'inventer un protocole maison
