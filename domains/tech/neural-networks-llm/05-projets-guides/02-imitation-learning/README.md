# Projet 02 — Imitation learning depuis traces d'operateurs experts

## Contexte metier

LogiSim a un corpus de traces de shifts ou des **operateurs OCC experts** ont pilote des flottes. L'idee : apprendre a une IA a piloter "comme eux" par imitation, pour enrichir les flottes autonomes de FleetSim avec des comportements plus subtils que ceux de AutonomyAI SDK (qui est base sur des regles).

C'est du **behavioral cloning** : on traite les decisions des operateurs comme un dataset supervise (etat -> action), et on entraine un modele a predire l'action donnee l'etat.
...
Entrainer un modele sequentiel (LSTM ou petit Transformer) qui, etant donne l'historique recent d'une flotte (positions, congestion locale, batterie), predit la **prochaine action a prendre** parmi un set discret : `MOVE_FORWARD`, `HOLD_POSITION`, `PICKUP`, `RETURN_TO_DOCK`, `WAIT`, `REPORT`.
...
Format :
```python
state_t = (pos_x, pos_y, congestion_dist, parcels_nearby, battery, payload, time_since_last_order)  # 7 floats
action_t = int in [0..5]
...
Si tu utilises ton modele pour **generer** une sequence (piloter une flotte), les erreurs s'accumulent : au step 1 l'etat est "normal", mais une mauvaise prediction emmene a un etat legerement off-distribution, puis de plus en plus, et le modele diverge completement. C'est le cas classique de BC.
...
- Publier le modele en onnx pour inference C++ cote FleetSim
