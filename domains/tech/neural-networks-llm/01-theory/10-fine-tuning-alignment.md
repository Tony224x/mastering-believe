...
RLHF (Christiano et al., 2017 ; premieres applications aux LLMs des 2019-2020 avec Ziegler et al. 2019 et Stiennon et al. 2020, puis recette popularisee par InstructGPT en 2022) est la methode qui a rendu ChatGPT possible. Elle est en 3 etapes :
...
```
L_IPO = E[ (log(pi_theta(y_w) / pi_ref(y_w)) - log(pi_theta(y_l) / pi_ref(y_l)) - 1/(2*tau))^2 ]
```
...
```
r_simpo(x, y) = (beta / |y|) * log pi_theta(y | x)              # length-normalized
L_SimPO = -log sigma(r_simpo(x, y_w) - r_simpo(x, y_l) - gamma)  # gamma = marge cible
```

**Avantages** :
- Pas de reference model (economie memoire 50%)
- Normalisation par la longueur (compense le biais DPO vers les reponses longues)
...
- **Tulu 3** (Allen Institute for AI, Lambert et al. 2024) : recette post-training open-source qui a formalise le terme RLVR (voir aussi DeepSeek-R1, module 15)
