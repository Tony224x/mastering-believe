...
def ascii_scatter(points_2d, labels, width=70, height=25, title=""):
    ...
    """
    # We need to map continuous coordinates to a discrete zone
    xs = points_2d[:, 0]
    ys = points_2d[:, 1]
    ...
    # Create empty zone — each cell is a string
    zone = [[' ' for _ in range(width)] for _ in range(height)]

    # Place each label on the zone at its mapped position
    for i, (x, y) in enumerate(zip(xs, ys)):
        # Map continuous coords to zone indices
        ...
        # Write the label (truncate if it would overflow the zone)
