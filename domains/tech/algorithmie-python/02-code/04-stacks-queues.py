...
Run: python domains/tech/algorithmie-python/02-code/04-stacks-queues.py
...
# =============================================================================
# SECTION 5: QUEUE — BFS shortest path on a zone
# =============================================================================

def bfs_shortest_path(zone, start, target):
    """
    FIFO BFS on a 4-connected zone.
    ...
    if zone[start[0]][start[1]] == 1 or zone[target[0]][target[1]] == 1:
        return -1
    queue = deque([(start, 0)])
    visited = {start}
...
def demo_bfs_grid():
    print("\n" + "=" * 70)
    print("QUEUE: BFS shortest path on a zone (0 = free, 1 = wall)")
    print("=" * 70)
    zone = [
        [0, 0, 0, 0],
        [1, 1, 0, 1],
    ...
    assert bfs_shortest_path(zone, (0, 0), (3, 3)) == 6
