...
Run the whole file to execute every solution.
    ...
    python domains/tech/agentic-ai/03-exercises/solutions/14-capstone.py
"""
...
CORPUS = day14.CORPUS
AcmeResearcher = day14.AcmeResearcher
AcmeState = day14.AcmeState
...
class AcmeResearcherWithCritic(AcmeResearcher):
...
def solution_1() -> None:
    ...
    researcher = AcmeResearcherWithCritic(
        llm=llm,
...
@dataclass
class SwarmAcmeResearcher:
    """Alternative to AcmeResearcher with explicit handoffs, no supervisor."""
    ...
    def _run_agent(self, name: str, state: AcmeState) -> AcmeState:
        if name == "researcher":
...
def solution_2() -> None:
    ...
    day14._GLOBAL_TRACER = Tracer()
    llm = MockLLM()
    researcher = SwarmAcmeResearcher(
        llm=llm,
        retriever=TinyTfIdfRetriever(CORPUS),
        input_guard=InputGuardrail(),
        output_guard=OutputGuardrail(canary=CANARY_TOKEN),
...
def extended_run_eval(researcher: AcmeResearcher) -> list[dict]:
    results: list[dict] = []
