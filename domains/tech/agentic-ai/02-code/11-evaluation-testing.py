...
Run:
    python domains/tech/agentic-ai/02-code/11-evaluation-testing.py
"""
...
class FakeAgent:
    ...
    def run(self, task: str) -> AgentRun:
        ...
        if "revenue" in low and "acme" in low:
            run.trajectory = ["search_docs", "extract_number"]
            run.llm_calls = 3
            run.answer = "Acme revenue in 2025 is 820k euros."
            if self.buggy:
                # Bug: skip search_docs -> hallucinates a wrong number
                run.trajectory = ["extract_number"]
                run.answer = "Acme revenue in 2025 is 3.5 million euros."

        elif "webhook" in low:
            run.trajectory = ["search_docs"]
            run.llm_calls = 2
            run.answer = "Use POST /webhook with X-Acme-Signature header."
            if self.buggy:
        ...
        elif "acme-immo" in low:
            run.trajectory = ["search_docs"]
            run.llm_calls = 2
            run.answer = "acme-immo is a real estate price transparency platform in West Africa."
