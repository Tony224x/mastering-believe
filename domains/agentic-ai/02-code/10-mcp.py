...
def build_demo_server() -> MiniMCPServer:
    ...
    _NOTES = "Kalira ships AI consulting and SaaS. Main product: kalira-immo."
...
def demo() -> None:
    ...
    print("  greet('Anthony') =", client.call_tool("greet", {"name": "Anthony"}))

    print("\n--- 4. list + read resources ---")
    resources = client.list_resources()
    for r in resources:
        print(f"  - {r['uri']} ({r['name']})")
    content = client.read_resource("notes://kalira")
    print(f"  content of notes://kalira -> {content}")

    print("\n--- 5. list + get prompts ---")
    prompts = client.list_prompts()
    for p in prompts:
        print(f"  - {p['name']}: {p['description']}")
    messages = client.get_prompt("review", {"target": "kalira-immo business model"})
    for m in messages:
