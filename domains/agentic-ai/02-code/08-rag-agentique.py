...
CORPUS: list[tuple[str, str, str]] = [
    ...
    ("d2", "docs_produit",
     "Kalira's flagship product for West Africa is kalira-immo, a real estate "
    ...
    ("d4", "blog",
     "Kalira reported revenue of 820000 euros in 2025, up from 180000 euros in 2024. "
    ...
    ("d6", "jira",
     ...
     "the licencia platform that serves sports federations in production."),
    ("d7", "docs_api",
     "The Kalira API exposes a POST /webhook endpoint that accepts JSON events. "
     "The signature must be validated with the header X-Kalira-Signature."),
    ("d8", "docs_api",
     "Rate limit on the Kalira API is 60 requests per minute per API key. "
     "Exceeding the limit returns HTTP 429."),
    ("d9", "blog",
     "Kalira's main French competitor in AI consulting is Artefact, which reported "
...
class MockLLM:
    ...
    def _route(self, query: str) -> list[str]:
        ...
        if any(w in q for w in ["revenue", "ca", "market", "competitor", "growth"]):
            sources.append("blog")
        if any(w in q for w in ["product", "kalira", "immo", "launched", "founded"]):
...
def demo() -> None:
    ...
    complex_query = (
        "What is the revenue of Kalira in 2025 compared to its main competitor "
