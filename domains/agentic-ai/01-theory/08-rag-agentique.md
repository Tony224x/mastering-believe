...
```
Question : "Quel est le CA de Kalira en 2025 compare a celui de son plus gros concurrent,
...
3. Probablement trouver un chunk sur le CA de Kalira, mais pas sur les concurrents
4. Generer une reponse incomplete ou hallucinee

Les problemes :
- **Question composee** : 3 informations a chercher (CA Kalira, CA concurrent, difference)
...
LLM decompose →
  Sub-question 1 : "Quel est le CA de Kalira en 2025 ?"
  Sub-question 2 : "Qui est le plus gros concurrent de Kalira ?"
...
```
Sub-question : "Quel est le CA de Kalira en 2025 ?"
Retrieved chunks :
  1. "Kalira a leve 2M€ en 2024..."         ← grade : IRRELEVANT (ce n'est pas le CA)
  2. "Le chiffre d'affaires 2025 : 4.2M€"    ← grade : RELEVANT
  3. "Kalira cible les TPE et PME..."       ← grade : IRRELEVANT
```
...
```
Sub-question originale : "Quel est le CA de Kalira en 2025 ?"
→ Retrieve → 0 chunks relevants

LLM reformule → "Chiffre d'affaires revenue 2025 Kalira SAS"
→ Retrieve → 2 chunks relevants ✓
```

**Strategies de reformulation** :
- **Synonymes** : "CA" → "chiffre d'affaires", "revenue"
- **Expansion** : ajouter des termes de contexte ("Kalira SAS", "2025 annee")
...
```
Query : "Quel est le CA de Kalira en 2025 ?"

LLM genere :
  [RETRIEVE=yes]
  -> chunk1: "Kalira a leve 2M€ en 2024"        [RELEVANCE=no]
  -> chunk2: "CA 2025 : 4.2M€"                  [RELEVANCE=yes]
  Reponse : "Le CA de Kalira en 2025 est de 4.2M€."  [SUPPORTED=yes] [UTILITY=5]
```
...
```
Phase offline :
  1. Extraire les entites et relations de chaque doc (ex: (Kalira) --[produit]-> (Legaly))
...
```
Query : "Quelles startups soutenues par Kalira travaillent sur l'IA en Afrique de l'Ouest ?"

Dense RAG : fait un matching semantique, trouve 5 chunks qui parlent de Kalira ou d'IA, peine a faire le lien
GraphRAG  : traverse (Kalira)->[invest]->(X,Y,Z), filtre sur (region=Afrique de l'Ouest), retourne la liste
```
