"""
Training du detecteur de fratricide — correction.

Pipeline :
1. Baseline majoritaire (predict "pas fratricide" tout le temps)
...
def train_mlp(X_train: np.ndarray, y_train: np.ndarray,
    ...
    _set_seeds()
    model = FratricideMLP(in_dim=X_train.shape[1])
...
def main() -> None:
    ...
    print("=" * 60)
    print("DETECTION DE FRATRICIDE — correction")
    print("=" * 60)

    X, y = load_dataset(n_samples=5000, fratricide_rate=0.03, seed=SEED)
    ...
    # Observation : accuracy ~97% mais F1=0 -> tous les fratricides sont manques
