"""
Correction commentee — A* tactique pour SWORD.
...
def _in_bounds(grid: Grid, p: Point) -> bool:
    r, c = p
    return 0 <= r < len(grid) and 0 <= c < len(grid[0])


def _neighbors(grid: Grid, node: Point, allow_diagonal: bool) -> Iterable[tuple[Point, float]]:
    ...
    for dr, dc in moves:
        ...
        if dr != 0 and dc != 0:
            ...
            if grid[node[0] + dr][node[1]] == math.inf:
                continue
            if grid[node[0]][node[1] + dc] == math.inf:
                continue
...
def astar(grid: Grid, start: Point, goal: Point, allow_diagonal: bool = False) -> list[Point] | None:
    ...
    if not _in_bounds(grid, start) or not _in_bounds(grid, goal):
        return None
    if grid[start[0]][start[1]] == math.inf or grid[goal[0]][goal[1]] == math.inf:
        return None
